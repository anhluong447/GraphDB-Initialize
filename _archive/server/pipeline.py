...
def _run_pipeline_worker(repo_url: str, language: str, job_id: str):
    ...
    try:
        ...
        # Step 1: Start databases and clear old data ONLY IF it is a new project
        state.update_job(1, "Starting databases & preparing index...")
        step_start_databases()
        
        if state.codebase_path != codebase_path:
            print(f"[Pipeline] New project detected (Old: {state.codebase_path}, New: {codebase_path}). Clearing databases...")
            # Clear Neo4j
            from graph.neo4j_client import get_client
            ...
            print(f"[Pipeline] Resuming indexing for existing project: {codebase_path}. Keeping existing databases.")
